...
export async function GET(req: NextRequest) {
  ...
  if (type === 'installation') {
    ...
    csv = 'maintenance_id,base_id,site_code,country,install_year,report_year,active_count,inactive_count,active_rate,active_count_method,active_count_accuracy\n'
    csv += rows.map((r) =>
      toCsvRow([r.maintenance_id, r.base_id, r.installation_base.site_code, r.installation_base.country, r.installation_base.install_year, r.report_year, r.active_count, r.inactive_count, r.active_rate.toFixed(4), r.active_count_method, r.active_count_accuracy])
    ).join('\n')
  } else if (type === 'report') {
    const where: Record<string, unknown> = {
    ...
    if (yearParam) where.report_year = Number(yearParam)
    const rows = await prisma.activeMaintenance.findMany({ where, include: { installation_base: true } })
    csv = 'site_code,country,install_year,report_year,installed_count,active_count,inactive_count,active_rate\n'
    csv += rows.map((r) =>
      toCsvRow([r.installation_base.site_code, r.installation_base.country, r.installation_base.install_year, r.report_year, r.installation_base.installed_count, r.active_count, r.inactive_count, r.active_rate.toFixed(4)])
    ).join('\n')
  }
