...
async function main() {
  ...
  await createMaintenance(jp_total.base_id, 50, 2022, 38, null, 'confirmed_by_site', 'confirmed')
  await createMaintenance(jp_total.base_id, 50, 2023, 40, 38, 'confirmed_by_site', 'confirmed')
  await createMaintenance(jp_total.base_id, 50, 2024, 42, 40, 'confirmed_by_site', 'confirmed')

  // JP MTB maintenance records
  await createMaintenance(jp_mtb.base_id, 30, 2022, 22, null, 'confirmed_by_site', 'confirmed')
  await createMaintenance(jp_mtb.base_id, 30, 2023, 24, 22, 'confirmed_by_site', 'confirmed')

  // US Total maintenance records
  await createMaintenance(us_total.base_id, 40, 2022, 28, null, 'estimated_by_active_rate', 'site_estimated')
  await createMaintenance(us_total.base_id, 40, 2023, 18, 28, 'estimated_by_active_rate', 'site_estimated')
  await createMaintenance(us_total.base_id, 40, 2024, 30, 28, 'estimated_by_active_rate', 'site_estimated')

  // US NC maintenance
  await createMaintenance(us_nc.base_id, 40, 2022, 28, null, 'calculated_from_status', 'confirmed')
  await createMaintenance(us_nc.base_id, 40, 2023, 30, 28, 'calculated_from_status', 'confirmed')

  // DE Total maintenance
  await createMaintenance(de_total.base_id, 25, 2022, 18, null, 'confirmed_by_site', 'confirmed')
  await createMaintenance(de_total.base_id, 25, 2023, 20, 18, 'confirmed_by_site', 'confirmed')
  await createMaintenance(de_total.base_id, 25, 2024, 21, 20, 'confirmed_by_site', 'hq_estimated')

  // DE Area maintenance
  await createMaintenance(de_area.base_id, 25, 2023, 20, null, 'estimated_by_active_rate', 'hq_estimated')
